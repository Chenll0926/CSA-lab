...
func worker(startY, startX, endY, endX int, data func(y, x int) uint8, out chan<- [][]uint8) {
...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		for i := 0; i < threads; i++ {
			startY := height * i / threads
			endY := height * (i + 1) / threads

			outChan := make(chan [][]uint8)
			outChannels = append(outChannels, outChan)

			go worker(startY, 0, endY, width, immutableData, outChan)
		}
		for i := 0; i < threads; i++ {
			newPixelData = append(newPixelData, <-outChannels[i]...)
		}
	}
